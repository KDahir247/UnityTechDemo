...
namespace Tech.UI.Panel
{
    public class TitleScreen_Document : VisualElement
    {
        ...
        private void OnGeometryChange(GeometryChangedEvent evt)
        {
            ...
            _titleScreen.Q<Button>("Option_Button")
                .RegisterCallback<ClickEvent>(evnt => FadeToNewScreen(evnt, _optionScreen, _titleScreen));
            _titleScreen.Q<Button>("Support_Button")
                .RegisterCallback<ClickEvent>(evnt => FadeToNewScreen(evnt, _supportScreen, _titleScreen));
            _titleScreen.Q<Button>("Mail_Button")
                .RegisterCallback<ClickEvent>(evnt => FadeToNewScreen(evnt, _newsScreen, _titleScreen));


            //TODO Find a better solution to handle screen transition in sub Visual Element
            //Out Of Panel Click Handle
            _optionScreen.Q<VisualElement>("Core_Panel")
                .RegisterCallback<PointerEnterEvent>(evnt => OutOfBound = false);
            ...
            {
                if (OutOfBound) FadeToNewScreen(evnt, _titleScreen, _optionScreen);
            });
        ...
        {
            if (!_isTransitioning)
                FadeOut(evt, fadeOutTarget,
                    () => FadeIn(evt, fadeInTarget, fadeOutTarget, () => { _isTransitioning = false; }));
        }
        ...
        private void FadeIn<T>(T evt, VisualElement visualElementToFadeIn, VisualElement visualElementThatFadeOut,
            Action onComplete)
            where T : PointerEventBase<T>, new()
        {
            ...
            if (!_isTransitioning)
            {
                if (Application.isPlaying)
                {
                    _titleScreen.experimental.animation
                        ...
                        .Ease(Easing.Linear)
                        .OnCompleted(() => { _titleScreen.style.display = DisplayStyle.None; });
