...
namespace UnityEngine.GameFoundation.Sample
{
    ...
    public class ItemListsSample : MonoBehaviour
    {
        ...
        public void OnGameFoundationInitialized()
        {
            ...
            GameFoundationSdk.catalog.GetItems(m_AllInventoryItemDefinitions);

            // Add a couple of items to get us started
            for (var i = 0; i < k_NumInitialItems; ++i)
                GameFoundationSdk.inventory.CreateItem(m_AllInventoryItemDefinitions[i]);

            // Create 3 item lists to use in sample
            for (var i = 0; i < k_NumCollections; ++i) m_ItemLists.Add(GameFoundationSdk.inventory.CreateList());
            ...
            foreach (var item in listItemsText) item.gameObject.SetActive(true);
        ...
        private void SubscribeToGameFoundationEvents()
        {
            ...
            if (!GameFoundationSdk.IsInitialized) return;
        ...
        private void RefreshUI()
        {
            ...
            for (var i = 0; i < k_NumCollections; ++i)
            {
                ...
                if (itemList.Count == 0)
                {
                    ...
                    m_DisplayText.AppendLine(
                        "<i>Use the buttons below to create new items in the list or add existing ones.</i>");
                    m_DisplayText.AppendLine();
                }
                else
                {
                    foreach (var item in itemList) m_DisplayText.AppendLine(item.definition.displayName);
                }
