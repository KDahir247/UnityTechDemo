...
namespace UnityEngine.GameFoundation.Sample
{
    ...
    public class PropertiesSample : MonoBehaviour
    {
        ...
        private void SubscribeToGameFoundationEvents()
        {
            ...
            if (GameFoundationSdk.inventory is null || m_Sword is null || m_HealthPotion is null) return;
        ...
        public void TakeDamage()
        {
            ...
            var quantity = 0;
            if (m_Sword.TryGetMutableProperty("quantity", out var quantityProperty)) quantity = quantityProperty;

            if (quantity <= 0 || damage <= 0 || m_PlayerHealth < damage) return;
        ...
        ///     This only happens if there is at least one health potion in the inventory, and if the player's health is not maxed
        ///     out.
        /// </summary>
        public void Heal()
        {
            ...
            var quantity = 0;
            if (m_HealthPotion.TryGetMutableProperty("quantity", out var quantityProperty)) quantity = quantityProperty;

            if (quantity <= 0 || m_PlayerHealth >= 100) return;
        ...
        private void RefreshDamageAndHealButtons()
        {
            ...
            if (m_Sword != null)
            {
                if (m_Sword.TryGetMutableProperty("damage", out var damageProperty)) swordDamage = damageProperty;
            ...
            if (m_HealthPotion != null &&
                m_HealthPotion.TryGetMutableProperty("quantity", out var potionQuantityProperty))
